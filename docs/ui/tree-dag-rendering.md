...
## 8. 上色:`_append_graph`(同欄同色)+ 跨線用 `─`

```python
LANE_COLORS = ["bright_cyan", "bright_magenta", "bright_green", "bright_yellow",
               "bright_blue", "bright_red", "cyan", "magenta"]   # 8 色循環

def _append_graph(text, graph):
    for i, ch in enumerate(graph):
        if ch == " ": text.append(" ")
        else:         text.append(ch, style=LANE_COLORS[(i // 2) % len(LANE_COLORS)])
```

- **上色規則(同欄同色)**:第 `i` 個字元 → 它所在的欄 `i // 2` → `LANE_COLORS[欄 % 8]`。一條 branch 整段固定同一欄,所以直泳道 `│` 整列同色、分支好追;簡單、穩定。
- **跨線用 `─`(2026-06 review 決定)**:當一條水平 merge/fork 邊**跨過**一條垂直泳道時,該格四向皆連(`_U|_D|_L|_R`),`_GLYPH` 把它畫成 **`─`**(讓邊像一條連續橫線「蓋過」泳道),而不是 `┼`。所以 `├───╮` 一眼就是「從這欄拉到那欄」的一條線;被蓋過的泳道只在這一列少一格 `│`(上下列仍是 `│`,視覺自動補上)。
  - **比較**:先前試過「整條邊單色」,但那會把被跨過那格染成邊色、破壞同欄同色一致性;改用 `─`+同欄同色,邊靠「連續橫線」分辨、泳道靠「同欄同色」分辨,各司其職。
  - **唯一在意的取捨**:四向交叉只可能由「邊跨泳道」產生(沒有真正的 4-way 合流),所以把 `┼` 一律改成 `─` 是安全的;三向接頭(`├ ┤ ┬ ┴`)代表邊「接上/離開」某泳道,保留不變。
