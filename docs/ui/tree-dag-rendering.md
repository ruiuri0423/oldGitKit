...
渲染層(`_node_string` / `_conn_string`)除了字串,還輸出一份**逐格顏色欄位圖 `color[]`**;`render_graph` / `GraphCache` 因此回 `(graph, color, commit)`。

- **直泳道 = 同欄同色**:`color[2*欄] = 該欄`。一條 branch 整段固定同一欄 → 整列同色、好追。
- **跨線用 `─`**:水平邊跨過垂直泳道時該格四向皆連(`_U|_D|_L|_R`),`_GLYPH` 畫成 **`─`** 蓋過泳道(非 `┼`),所以 `├───╮` 是一條連續橫線;被蓋泳道只在這列少一格 `│`,上下列自動補上。四向只可能來自「邊跨泳道」(無真正 4-way 合流),故 `┼ → ─` 安全;三向接頭 `├ ┤ ┬ ┴`(邊接上/離開泳道)保留。
- **一條邊 = 一個顏色(側支色)**:`_conn_string` 把整條 move 的格染成**側支**(離主幹較遠的欄 `hi`)色,**只有接回主幹的角落**(`lo`)留主幹色。這樣一條 merge/fork 線整段和它所屬的分支同色、好追,而它接回去的那個 `├`/`╰` 仍是主幹色 —— 對應 commit 圖上「分支線回到主幹」的視覺。
  - 為什麼是 `hi`(側支)而非 `lo`:`_assign_columns` 一定把被併/分出的支放在比 basis 更外側的欄,所以兩端中「較外側 `hi`」恆為那條支、「較內側 `lo`」恆為主幹端。
  - 演進:先試「整條邊(含跨格)染**來源**色」→ 跨格用 `┼` 會破壞被跨泳道同色;改 `─` 跨線後,跨格已是邊專屬(泳道不顯示),於是回到「整條邊單色」但改用**側支色 + 主幹角落保留**,兩者兼得。
