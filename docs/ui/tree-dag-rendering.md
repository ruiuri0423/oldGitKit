...
- **規則**:第 `i` 個字元 → 它所在的欄 `i // 2` → 顏色 `LANE_COLORS[欄 % 8]`。上色**只看位置、不看這筆屬於哪條邊**。
- **為什麼這樣設計**:因為一條 branch 整段固定同一欄,所以直的泳道 `│` 會整列同色 → 分支顏色一致、好追;這是位置上色最大的好處,實作也最簡單。
- **已知副作用**:因為跨欄的連接邊會經過多個欄,而上色只看「字元站在第幾欄」,所以**一條 merge 邊會被它經過的每一欄分段染色**(例如 `├─┼─╮` 會是青/洋紅/綠三段),跨到的那欄是什麼色就染什麼色。若要「整條邊一個顏色」,得讓 `_specs` 額外輸出每格的 edge 身分;目前為求簡單維持位置上色。
